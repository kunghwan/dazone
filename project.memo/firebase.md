...
서버 만들기 어렵거나 귀찮을 때 프론트에서 다 하려고 만듬, 백, 프론트 상의 필요없이 프론트에서 다 짜면 됨

1. 영구성 부여를 위한 자료를 데이터베이스에 저장

MySQL, PosgreQL, LiteSQL, SQL(sequel)씨쿠얼
백앤드 프론트엔드에서 사용 불가 -> 서버 내용을 안전하게 보관하기 위함 -> Table 베이스의 Relational RDB

java Spring Boot, FastApi, deno, C#, .NET
...
PERN
PostgreQL + Express + React + Node
prisma js library

DB + NextJs
React + Server

Superbase + MongoDb + Firebase + Redis + Next

Next(RRD XX) SSR
Server + DB + front

RN
IOS ANDROID api => Nextjs

REACT js

- Electron 데스크탑 프로그램 javascript -> React도 가능

<!-- await를 쓰는이유 파이어베이스에서 데이터를 이동하기 위해 쓰이는거 택시탈떄 버스탈때 같은걸로생각 데이터 저장 -->

## 인증 Auth 오쓰(Authentication) 오뗀티케이션

비밀번호 관리가 귀찮아서 다른 곳에 맡김
3rd Party Auth 라고 부름 0Auth

firebase 제 3자 인증업체 -> 제 4제에게 인증을 한 번 더 요청할 뿐

1. 유저 감지하기
   useEffect + auth.onAuthStateChanged를 사용해서 유저에 uid를 받아오는 방식을 취함
   uid롤 파이어스토어에 있는 특정user를 가져오면 됨
...
인증의 회원가입 기능을 통해서 회원가입 후 회원정보를 별도로 데이터베이스에 저장함
...
5GB까지 무료, 그 이상은 과금 됨
과금이 되야 먹고살기 때문에 신용카드 정보 입력해야 사용할 수 있음
Spark 요금제에서 Blaze 요금제로 이동해야만 함

Blaze요금제도 Spark 요금제에서 제공하는 무료 사용량 까지는 무료로 사용할 수 있음
**무료요금제 용량 차고 넘침**

한글기준 1글자의 용량 = 3bytes 가=3 가나=6
기본 이미지 사이즈 1Mb = 105만 bytes = 35만개의 글자와 동일
